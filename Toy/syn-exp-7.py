...
sharps = np.logspace(-1,3,96)
sharp = sharps[task_id]

stds = np.linspace(0, 3, 100)
lrs = np.logspace(-3, 0, 20)
losses = np.zeros((3, 100, 20, 50))
    ...
    for std_idx in range(100):
        ...
        for lr_idx in range(20):
            ...
            for replicate in range(50):
                run_optim = Runexp(p_landscapes = (sharp,.1), lr = lr, optimizer = optimizer, betas = (0.9, beta2), 
                                   gamma = gamma, epsilon = 1e-8, theta = np.pi/6, weight_decay=0.1)
                for step in range(1000):
                    run_optim.step(std = std)
                losses[optim_idx,std_idx,lr_idx,replicate] = run_optim.loss()
    
        print(f'To optimizer: {optimizer}, std_idx = {std_idx}')

print(f'Total time: {time.time() - start_time}')
np.save(f'../outputs/syn-exp-7-{task_id}.npy', losses)